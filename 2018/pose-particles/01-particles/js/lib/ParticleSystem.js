...
export default function(Vector, Field, Emitter, Particle) {
  ...
  _.extend(ParticleSystem.prototype, Backbone.Events, {
    ...
    onAfterDraw: function(evt) {
      if (this.display.draw.info) {
        ...
        this.display.drawText(
          ...
          new Vector(100, this.display.height - 10),
          100
        )
      }
    },
    ...
    onMouseMove: function(evt) {
      this.mouseCoords = new Vector(
        evt.offsetX || evt.layerX - this.display.canvas.offsetLeft,
        evt.offsetY || evt.layerY - this.display.canvas.offsetTop
      )
      if (this.mouseField) {
        ...
        // not over anything
        var object = this.getObjectAtPoint(this.mouseCoords)
        if (this.objectMouseOver !== object) {
          // if we're over something different
          if (this.objectMouseOver) {
            // if we were over something before
            evt.particleTarget = this.objectMouseOver
            this.trigger('objectMouseOut', evt)
            this.objectMouseOver = undefined
          } else {
            // we're in *something* new, even if it's nothing
            evt.particleTarget = object
    ...
    drawAccelerations: function() {
      ...
      _(this.particles).each(function(particle) {
        ...
        display.context.lineTo(
          particle.position.x + particle.acceleration.x,
          particle.position.y + particle.acceleration.y
        )
      })
    ...
    drawFields: function() {
      _(this.fields).each(this.drawCircularObject.bind(this))
    },
    drawEmitters: function() {
      _(this.emitters).each(this.drawCircularObject.bind(this))
    },
    ...
    getObjectAtPoint: function(point) {
      var objects = _([].concat(this.emitters, this.fields)).filter(function(
        obj
      ) {
    ...
    toString: function() {
      ...
      _(this.emitters).each(function(o) {
        coreAttributes.push(o.toString())
      })
      _(this.fields).each(function(o) {
        coreAttributes.push(o.toString())
      })
